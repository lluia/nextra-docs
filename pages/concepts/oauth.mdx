...
<Callout>
  NextAuth.js is designed to work with any OAuth service, it supports **OAuth
  1.0**, **1.0A**, **2.0** and **OpenID Connect** and has built-in support for
  most popular sign-in services.
</Callout>

Authentication Providers in **Auth.js** are [OAuth](https://oauth.net/2/) definitions that allow your users to sign in with their favorite pre-existing logins. You can use any of our many predefined providers, or write your own custom OAuth configuration.

- [Using a built-in OAuth Provider](/overview/authentication#setup-oauth) (e.g Github, Twitter, Google, etc...)
- [Using a custom OAuth Provider](/tutorials/custom-oauth)

Without going into too much detail, the <u>OAuth flow generally has 6 parts</u>:
...
Here is a more visual representation 👀 of a typical OAuth flow with Auth.js ( _this example is with Sveltekit, but the same concepts should apply to any framework_ ):

<br />
<br />
<img src="/img/concepts/oauth-diagram.webp" />
