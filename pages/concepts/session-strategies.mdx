...
When a user visits your application, you want them to not have to log in every single time. This is called a session. <u>Auth.js supports 2 main session strategies</u>, JWT session and Database session, which are described below.

<Callout>
  Both session strategies have advantages and disadvantages which you have to
  evaluate based on the requirement of your application.
</Callout>

Controlling the session strategy can be done through the [`session.strategy`](/reference/core#session) configuration option in the main Auth.js config file.

## JWT Session

Auth.js can create sessions using [JSON Web Tokens (JWT)](https://datatracker.ietf.org/doc/html/rfc7519). This is the default session strategy for Auth.js. When a user signs in, a JWT is created [in a `HttpOnly` cookie](https://developer.mozilla.org/en-US/docs/Web/HTTP/Cookies#restrict_access_to_cookies). Making the cookie `HttpOnly` prevents JavaScript from accessing it client-side (`document.cookie`), which makes it harder for attackers to steal the value. In addition, the JWT is encrypted with a secret key only known to the server. So even if an attacker were to steal the JWT from the cookie, they would not be able to decrypt it. Combined with a short expiration time, this makes JWTs a secure way to create sessions.
...
- Auth.js has no opinion on the type of database you are using, we have a big list of [official database adapters](/getting-started/database-setup), but you can [implement your own](/tutorials/cusomt-adapter) as well
