...
{/* TODO: This whole page needs to have a similar stepper to the Authentication section for providers */}

# Setup a database

<Callout type="info">
  Auth.js integrations save sessions in a cookie by default. So seting up a database is optional. However, if you
...
_Database Adapters_ are the bridge we use to connect to your database. For instance, [when implementing magic links](/guides/passwordless-sendgrid-setup), the `EmailProvider` will require you to setup a database adapter to be able to store the [verification tokens](/concepts/database-models#verificationtoken-model) present on the links.
...
<Callout>
  The database schema is not required to be exactly as shown below. You can add or remove fields
  as you see fit. However, if you do, you'll need to update the database adapter to match the
  schema you've defined. More details on what each model is used for and if you need it can be found [here](/guides/creating-a-database-adapter).
</Callout>
...
<Callout>
  ...
  adapter](/guides/creating-a-database-adapter). If you create a new adapter, we'd love it
  if you [opened a PR](/guides/creating-a-database-adapter#official-adapter-guidelines) to share it with everyone!
</Callout>
