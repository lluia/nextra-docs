...
<Callout type="info">
  Installing `@auth/core` is not necessary.
  As a consumer of the library, you should never need to install it.
</Callout>
...
`AUTH_SECRET` is a mandatory, random value used by the library to encrypt tokens and email verification hashes. (See [Deployment](/getting-started/deployment) to learn more). You can generate one via running:
