...
<ListDisclosure limit={8}>
  {Object.entries(manifest.providers).map(([id, name]) => (
    <Link
      ...
      key={name}
      className="p-4 border border-solid dark:border-neutral-800 dark:bg-neutral-950
      border-slate-200 rounded-lg flex flex-col items-center justify-between w-36 shadow-lg h-32"
    >
      <div className="grid place-items-center flex-grow">
        <img src={`/img/providers/${id}.svg`} className="w-11 mt-2" />
      </div>
      <div className="text-sm text-center break-word-legacy !text-wrap">
        {name}
      </div>
    </Link>
  ))}
...
<Callout>
  ...
  application, check out [session
  management](/getting-started/session-management) for more details.
</Callout>
