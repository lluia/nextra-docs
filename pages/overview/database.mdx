...
<Callout>
Auth.js uses JSON Web Tokens by default for saving the user's session so **you don't need to setup a
database for authenticating users**. However, if you want to persist user information in your own database, or you want to implement certain flows, you can use a Database Adapter.
</Callout>


**Database Adapters** are packages maintained by the Auth.js team to connect Auth.js to the most common databases.

For instance, [when implementing magic links](/tutorials/passwordless-sentry-setup), the `EmailProvider` will require you to setup a database adapter to be able to store the [verification tokens](/concepts/database-models#verificationtoken-model) present on the links.
...
<Callout>
  ...
  adapter](/tutorials/custom-adapter). If you create a new adapter, we'd love
  it if you created a PR to our repo to share it with everyone!
</Callout>
