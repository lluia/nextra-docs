...
import { GitDiff, ChatText, Password } from "@phosphor-icons/react";
import { ListDisclosure } from "@/components/ListDisclosure";
import manifest from "@/data/manifest.json";

# Authentication

Before start setting up Auth.js, you need to decide how you're gonna authenticate users in your application. Auth.js supports <u>several authentication paradigms</u>:

- **OAuth** (_Sign in with Google, Github, LinkedIn, etc..._)
- **Token-based** ( _Magic links, OTP codes, etc..._ )
- **Credentials** ( _External Auth, Webauthn, etc..._ )
  ...
  <RichTabs.TabList>
    <RichTabs.Tab key="oauth" value="oauth">
      <div className="flex flex-col gap-1 justify-center items-center h-full">
        <GitDiff fontSize="2.2rem" />
        <span>Setup OAuth</span>
      </div>
    </RichTabs.Tab>
    <RichTabs.Tab key="token-auth" value="token-auth">
      <div className="flex flex-col gap-1 justify-center items-center h-full gap-1">
        <ChatText fontSize="2.2rem" />
        <span>Setup Token auth</span>
      </div>
    </RichTabs.Tab>
    <RichTabs.Tab key="external-auth" value="external-auth">
      <div className="flex flex-col gap-1 justify-center items-center h-full">
        <Password fontSize="2.3rem" />
...
<Callout type="info">
  ...
  OAuth provider as long as they support **OAuth/OIDC**.
</Callout>
...
Auth.js (with Next.js) will automatically pick up these. You can [also use a different name for the environment variables](/concepts/environment-variables#oauth-variables) if needed, but then you'll need to pass them to the provider manually. All other client libraries also require you to manually pass the environment variables to the provider.
...
Now you should be able to test signing in. Start your application, and visit `/api/auth/signin`. That route will load the default Auth.js signin page.

import OAuthOptions from "../../public/img/getting-started/oauth-options.webp";

<Screenshot src={OAuthOptions} alt="OAuth Options" />
...
<Callout>
  ...
  application, check out [session management](/overview/session-management) for
  more details.
</Callout>
...
<RichTabs.Panel key="token-auth" value="token-auth">
...
Please make sure you've [setup a database adapter](/overview/database-setup), as mentioned earlier,
...
<Callout>
  ...
  Sentry](/tutorials/deep-dive/passwordless-sentry-setup) for a better
...
your setup is working. See the next section, [Session management](/overview/session-management),
