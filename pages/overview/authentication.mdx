...
<Callout type="info">
  ...
  Auth.js can be setup with any OAuth provider as long as they support **OAuth
  2**.
</Callout>
...
In this case, given we want to try our application locally, it would be (the provider's name is always in [kebab case](https://www.theserverside.com/definition/Kebab-case), so for Github → `github`):
...
Auth.js will automatically pick up these. You can [also use a different name if needed](/concepts/environment-variables#oauth-variables).

Now start your application and navigate to `/signin`. Click on `Sign in with GitHub`, if all went well you should have signed in with Github (or whatever provider you chose) via OAuth.
