...
- **Token-based** (_Magic links, WebAuthn, etc..._)
- **Credentials** (formerly known as CredentialProvider, _Integrating with external APIs, etc..._)
...
<ListDisclosure limit={8}>
  {Object.entries(manifest.providers).map(([id, name]) => (
    <Link
      ...
      key={name}
      className="p-4 border border-solid dark:border-neutral-800 dark:bg-neutral-950 border-slate-200 rounded-lg flex flex-col items-center justify-between w-36 shadow-lg h-32"
    >
