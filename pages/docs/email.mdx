...
{/** Needs to be moved to under /docs/authentication token auth -> nodemailer **/}
...
<Callout type="info">
  ...
  sessions, but you [must configure a database adapter](/docs/database) to use
  it. It is not possible to enable email sign in without using a database.
</Callout>
