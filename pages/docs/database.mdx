...
implementing magic links](/docs/passwordless-sendgrid-setup), the `EmailProvider` will require you to setup a database adapter to be able to store the [verification tokens](/concepts/database-models#verificationtoken-model) present on the links.
...
<Callout>
  ...
  [here](/docs/creating-a-database-adapter).
</Callout>
...
<Callout>
  ...
  adapter](/docs/creating-a-database-adapter). If you create a new adapter, we'd
  love it if you [opened a
  PR](/docs/creating-a-database-adapter#official-adapter-guidelines) to share it
  with everyone!
</Callout>
